...
class Prodigy(torch.optim.Optimizer):
    ...
    Arguments:
        ...
        beta3 (float):
            Coefficient for computing the Prodidy stepsize using running averages (default: None).
        ...
        use_bias_correction (boolean):
            Turn on Adam's bias correction (default: False)
        safeguard_warmup (boolean):
            Remove lr from the denominator of D estimate to avoid issues during warm-up stage (default: False)
        d0 (float):
        ...
        growth_rate (float):
            Prevent the D estimate from growing faster than this multiplicative rate (default: inf).
            Default is inf, for unrestricted; values like 1.02 give a kind of lr warmup effect
        fsdp_in_use (bool):
            If you're using sharded parameters, this should be set to True.
            The optimizer will attempt to auto-detect this, but if you're using an implementation other
