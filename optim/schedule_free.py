...
class AdamWScheduleFree(torch.optim.Optimizer):
    ...
    Arguments:
        ...
        r (float):
            Use polynomial weighting in the average with power r (default 0)
        ...
        foreach (bool):
            Use a foreach-backed implementation of the optimizer. Should be
            significantly faster, but will have higher peak memory usage
            (default True, if supported in your PyTorch version)
    """
...
class SGDScheduleFree(torch.optim.Optimizer):
    ...
    Arguments:
        ...
        momentum (float):
            Momentum factor, must be between 0 and 1 exclusive (default: 0.9)
        weight_decay (float):
            Weight decay, i.e. a L2 penalty (default: 0)
        warmup_steps (int):
            Enables a linear learning rate warmup (default 0)
        ...
        foreach (bool): Use a foreach-backed implementation of the optimizer.
            Should be significantly faster, but will have higher peak memory usage
            (default True if supported in your PyTorch version)
    """
